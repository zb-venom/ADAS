...
bool make_user_code_req(uint8_t *serv, byte code [16], byte user_code[16], int type) { 
        ...
        if (client.connect(serv, 80)) {
            ...
            char add[3] = {'a','d','d'};
            char get[3] = {'g','e','t'};
            char lol2[10] = {'.','p','h','p','?','c','o','d','e','='};            
            char lol3[11] = {'&','u','s','e','r','_','c','o','d','e','='};
            char kek[54] = { ' ', 'H','T','T','P','/','1','.','1','\r','\n','H','o','s','t',':',' ','1','9','2','.','1','6','8','.','1','0','0','.','1','1','\r','\n','C','o','n','n','e','c','t','i','o','n',':',' ','c','l','o','s','e','\r','\n','\r','\n'};
            char temp[115]; int i = 0;
            //заполнение массива данными о запросе и данными 
            while (i < 5) {temp[i]=lol[i]; i++;} i = 0; 
            if (type == 1){
                while (i < 3) {temp[i+5]=get[i]; i++;} 
            }
            else                
                while (i < 3) {temp[i+5]=add[i]; i++;}        
            i = 0;
            while (i < 10) {temp[i+8]=lol2[i]; i++;} i = 0;
            while (i < 16) {temp[i+18]=code[i]; i++;} i = 0; 
            while (i < 11) {temp[i+34]=lol3[i]; i++;} i = 0;            
            while (i < 16) {temp[i+45]=user_code[i]; i++;} i = 0; 
            while (i < 54) {temp[i+61]=kek[i]; i++;}       
...
bool Ethernet_request ::intialazion_server(byte user_device[16], byte user_login[16], int type)
{
        Delay (5000);
        uint8_t mac[] = { 0x90, 0xAD, 0xDA, 0x0D, 0x96, 0xFE };// Mac - адрес сервера
        uint8_t serv [] = { 192, 168, 100, 11 };//адрес сервера 
        ...
        if(make_user_code_req(serv, user_device, user_login, type)==false){ // отрправка дынных на сервера
            return false;
        }
